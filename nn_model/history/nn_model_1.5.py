...
def backward_propagation(Y, parameters, caches, activation, lambd, keep_prob):
    ...
    grads["dW"+str(L)] = (1./m) * np.dot(caches["dZ"+str(L)], caches["A"+str(L-1)].T) +\
    ...
    for i in reversed(range(1, L)):
        ...
        grads["dW"+str(i)] = (1./m) * np.dot(caches["dZ"+str(i)], caches["A"+str(i-1)].T) +\
...
def control_strategy(label):
    ...
    optimizer = 3                 # 1:gradient; 2:momentum; 3:adam

    # 策略一：一个隐藏层，不同的单元数
    if label == 1:
        num_unit = [50, 100, 200, 300]
        func_flag = [1, 0, 0]
