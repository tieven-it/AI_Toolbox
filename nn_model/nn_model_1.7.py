...
import copy
import scipy.io
from models.activation import relu, relu_gradient,\
...
def load_cat_dataset():
    ...
    # 将y转换为0/1矩阵
    train_y = np.eye(len(classes))[train_y_orig].T
    test_y = np.eye(len(classes))[test_y_orig].T
...
def forward_propagation(X, Y, parameters, activation, lambd, keep_prob):
    ...
    cost_part1 = (-1./m) * np.sum(Y * np.log(caches["A"+str(L)]))  
...
def gradient_checking(activation, lambd, keep_prob):
    ...
    Y = np.squeeze(np.random.randint(0, 2, (1, m)))             # 随机初始化m个标签
    Y = np.eye(2)[Y].T
    lim = 1e-4                                                  # 误差值
    layer_dims = [n, 5, 5, 3, 2]                                # 神经网络规格
...
def predict(X, Y, parameters, activation, lambd=0, keep_prob=1):
    ...
    index_A = np.argmax(caches["A"+str(L)], 0)
    index_Y = np.argmax(Y, 0)
    prediction = np.mean(index_A == index_Y) * 100
...
def plot_decision_boundary(X, Y, parameters, activation, lambd=0, keep_prob=1):
    ...
    test_y = np.squeeze(np.random.randint(0, 2, (1, test_x.shape[1])))  
    test_y = np.eye(2)[test_y].T                                   
    
    # 获得预测值
    caches, _ = forward_propagation(test_x, test_y, parameters, activation, lambd, keep_prob)
    Z = np.argmax(caches["A"+str(L)],0)
    Z = Z.reshape(xx.shape)
    
    # 画图
    y_temp = np.argmax(Y,0)
    plt.contourf(xx, yy, Z, cmap=plt.cm.Spectral)
    plt.scatter(X[0, :], X[1, :], c= y_temp, s=40, cmap=plt.cm.Spectral)
    plt.ylabel('x2')
    plt.xlabel('x1')
    plt.show()
...
def nn_model(alpha, maxIters, layer_dims, activation, lambd, keep_prob, batch_size, optimizer):
    ...
    # 导入数据
    train_x, train_y, test_x, test_y = load_signs_dataset()
...
def strategy_1(history, func, alpha, maxIters, func_flag, num_unit, num_features,
                lambd, keep_prob, batch_size, optimizer, classes):
    cnt = 0
    ...
    for i in range(len(num_unit)):
        for j in range(len(func_flag)):
            ...
            layer_dims = [num_features, num_unit[i], classes]
            activation = [func[j], "softmax"]
...
def strategy_2(history, func, alpha, maxIters, func_flag, num_unit, num_layer, 
                num_features, lambd, keep_prob, batch_size, optimizer, classes):
    ...
    for i in range(len(num_layer)):
        ...
        layer_dims.append(classes)

        for k in range(len(func_flag)):
            activation = [func[k], "softmax"]
...
def control_strategy(label):
    ...
    if label == 3:
        ...
        layer_dims = [num_features,30,30,classes]
        history["test0"] = {}
    ...
    for i in range(len(history)):
        ...
        print("prediction_test:%.2f" % history["test"+str(i)]["prediction_test"])
        print("prediction_train:%.2f" % history["test"+str(i)]["prediction_train"])
        print("score:%.2f" % history["test"+str(i)]["score"])
